	...
	if (item.type === 'file' && item.file) {
		return (
			<button
				...
				className={`flex items-center w-full gap-2 py-1 px-3 transition-colors text-sm rounded-[0.625rem] ${
					isCurrentFile
						? 'text-accent bg-accent/10'
						: 'text-text-primary/80 hover:bg-neutral-100 dark:hover:bg-bg-3 hover:text-text-primary'
				}`}
	...
	return (
		<div>
			<button
				...
				className="flex items-center gap-2 py-1 px-3 transition-colors text-sm text-text-primary/80 hover:bg-neutral-100 dark:hover:bg-bg-3 hover:text-text-primary w-full rounded-[0.625rem]"
				style={{ paddingLeft: `${level * 12 + 12}px` }}
			>
				<ChevronRight
					className={clsx(
...
export function FileExplorer({
	...
	return (
		<div className="w-full max-w-[200px] bg-white dark:bg-bg-2 border-r border-neutral-300 dark:border-neutral-700 h-full overflow-y-auto">
