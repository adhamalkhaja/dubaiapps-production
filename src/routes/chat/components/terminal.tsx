...
export function Terminal({ 
	...
	return (
		<div className={clsx(
			...
			{/* Terminal Output */}
			<div className="flex-1 min-h-0 overflow-hidden bg-neutral-50 dark:bg-[#1d1e1e]">
				<ScrollArea 
					ref={scrollAreaRef}
						...
						{logs.length === 0 ? (
							...
							logs.map((log) => (
								<div
									key={log.id}
									className={clsx(
										'group flex items-start gap-3 py-1.5 px-2 -mx-2 rounded-[0.625rem]',
						...
						{isConnected ? (
							<div className="flex items-center gap-4">
								<span className="flex items-center gap-1">
									Press 
									<kbd className="px-1.5 py-0.5 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-[0.375rem] text-xs font-mono">↑</kbd>
									<kbd className="px-1.5 py-0.5 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-[0.375rem] text-xs font-mono">↓</kbd>
									for history
								</span>
							</div>
