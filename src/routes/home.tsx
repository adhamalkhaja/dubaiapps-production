...
export default function Home() {
	...
	return (
		<div className="relative flex flex-col items-center size-full bg-[#fafafa] dark:bg-[#020202]">
			...
			<LayoutGroup>
				<div className="rounded-md w-full max-w-2xl overflow-hidden">
					<motion.div
						...
						<h1 className="font-sans text-[64px] md:text-5xl lg:text-[64px] leading-[1.1] font-normal tracking-tight text-black dark:text-white w-full mb-4">
						...
						<form
							...
							className="flex z-10 flex-col w-full min-h-[150px] bg-white dark:bg-bg-2 border border-neutral-300 dark:border-neutral-800 rounded-[0.625rem] shadow-sm p-5 transition-all duration-200"
						>
							<div 
								className={clsx(
							...
							>
								{isDragging && (
									<div className="absolute inset-0 flex items-center justify-center bg-accent/10 backdrop-blur-sm rounded-[0.625rem] z-30 pointer-events-none">
							...
							<div className="flex items-center justify-between mt-4 pt-1">
								...
								<button
									type="submit"
									disabled={!query.trim()}
									className="bg-accent text-white p-1 rounded-[0.625rem] *:size-5 transition-all duration-200 hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
								>
									<ArrowRight />
								</button>
							</div>
				...
				<AnimatePresence>
					{images.length > 0 && (
						...
						>
							<div className="flex items-start gap-2 px-4 py-3 rounded-[0.625rem] bg-white dark:bg-bg-2 border border-neutral-300 dark:border-neutral-800 shadow-sm">
								<Info className="size-4 text-accent flex-shrink-0 mt-0.5" />
								<p className="text-xs text-neutral-600 dark:text-text-tertiary leading-relaxed">
									<span className="font-medium text-black dark:text-text-secondary">Images Beta:</span> Images guide app layout and design but may not be replicated exactly. The coding agent cannot access images directly for app assets.
								</p>
							</div>
						</motion.div>
					)}
					...
					{discoverReady && (
						...
						>
							<div className='flex flex-col items-start'>
								<h2 className="text-2xl font-medium text-black dark:text-white">Discover Apps built by the community</h2>
								<div ref={discoverLinkRef} className="text-md font-light mb-4 text-neutral-600 dark:text-text-tertiary hover:underline underline-offset-4 select-text cursor-pointer" onClick={() => navigate('/discover')} >View All</div>
								<motion.div
									layout
